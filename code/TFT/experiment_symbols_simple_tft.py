#!/usr/bin/env python3
"""
SimpleTFT implementation for multi-GPU CUDA streams training
Compatible with existing training infrastructure and Numba streams
"""
...
try:
    ...
    print(" Numba not available - CUDA streams will be disabled")
...
RESULTS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'simple_tft_results')
MODELS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'simple_tft_models')
...
def train_and_evaluate_on_stream(csv_file, stream, device_id=0, stream_id=1):
    """Train and evaluate SimpleTFT model on a single stock CSV file using a specific stream"""
    # Extract stock symbol from filename
    symbol = os.path.basename(csv_file).split('.')[0]
    region = os.path.basename(os.path.dirname(csv_file))

    print(f"Processing {region}/{symbol} on stream{stream_id}")
    ...
    # Save model
    model_path = os.path.join(MODELS_DIR, f"{region}_{symbol}_simple_tft_model.pt")
    torch.save({
...
def process_gpu(gpu_id, csv_files):
    """Process training for a specific GPU using streams"""
    ...
    results = []
    print(f"Processing GPU {gpu_id} with {len(csv_files)} symbols using SimpleTFT")

    # Create streams for this GPU
    num_streams = min(len(csv_files), 8)
    if NUMBA_AVAILABLE:
    ...
    for i in range(0, len(csv_files), num_streams):
        batch_files = csv_files[i:i+num_streams]
        batch_streams = pytorch_streams[:len(batch_files)]
        num_batches += 1

        print(f"  Starting batch {num_batches} with {len(batch_files)} symbols")
        ...
        with ThreadPoolExecutor(max_workers=len(batch_files)) as thread_executor:
            futures = [
                thread_executor.submit(train_and_evaluate_on_stream, csv_file, stream, 0, stream_id=j+1)
                for j, (csv_file, stream) in enumerate(zip(batch_files, batch_streams))
            ]
    ...
    if NUMBA_AVAILABLE:
        for stream in streams:
            stream.synchronize()
            # PyTorch CUDA streams are auto-managed, no close() method needed
    else:
...
def run_parallel_experiment(num_symbols=4, multi_gpu=False):
    """Run parallel training experiment using SimpleTFT"""
    ...
    if multi_gpu and num_gpus > 1:
        print(f"Using {num_gpus} GPUs for SimpleTFT parallel training")
        symbols_per_gpu = num_symbols // num_gpus
        remaining = num_symbols % num_gpus
    else:
        num_gpus = 1
        symbols_per_gpu = num_symbols
        remaining = 0
        print("Using single GPU (GPU 1) for SimpleTFT parallel training")
    ...
    print(f"Selected {num_symbols} symbols for SimpleTFT training")

    # Distribute symbols across GPUs
    gpu_assignments = []
    start_idx = 0
    for gpu_id in range(num_gpus):
        count = symbols_per_gpu + (1 if gpu_id < remaining else 0)
        # Use GPU 1 for single GPU mode, otherwise use the actual GPU ID
        actual_gpu_id = 1 if not multi_gpu else gpu_id
        gpu_assignments.append((actual_gpu_id, selected_csv_files[start_idx:start_idx + count]))
        start_idx += count
    ...
    with ProcessPoolExecutor(max_workers=num_gpus) as executor:
        futures = [executor.submit(process_gpu, gpu_id, csv_files) for gpu_id, csv_files in gpu_assignments]
        for future in as_completed(futures):
    ...
    total_streams = sum(gpu['num_streams'] for gpu in gpu_timing_data)

    print(f"\nSimpleTFT parallel training completed in {total_time:.2f} seconds")

    # Calculate theoretical sequential time for comparison
    if all_results:
        avg_individual_time = sum(r['metrics']['training_time'] for r in all_results if 'metrics' in r and 'training_time' in r['metrics']) / len([r for r in all_results if 'metrics' in r and 'training_time' in r['metrics']])
        ...
        print(f"\n{'='*50}")
        print(f"{'PARALLEL PERFORMANCE SUMMARY':^50}")
        print(f"{'='*50}")
        print(f"{'Metric':<25} | {'Value':<12} | {'Unit'}")
        print(f"{'-'*50}")
        print(f"{'Total Experiment Time':<25} | {total_time:<12.2f} | seconds")
        print(f"{'Parallel Speedup':<25} | {speedup:<12.2f} | factor")
        print(f"{'='*50}")
    else:
        print(f"\n{'='*50}")
        print(f"{'PARALLEL PERFORMANCE SUMMARY':^50}")
        ...
        print(f"{'-'*50}")
        print(f"{'Total Experiment Time':<25} | {total_time:<12.2f} | seconds")
        print(f"{'Parallel Speedup':<25} | {'N/A':<12} | factor")
        print(f"{'='*50}")
    ...
    if successful_results:
        ...
        results_path = os.path.join(RESULTS_DIR, f'simple_tft_results_{num_symbols}_symbols.csv')
        results_df.to_csv(results_path, index=False)
        
        # Save detailed results
        detailed_path = os.path.join(RESULTS_DIR, f'simple_tft_detailed_{num_symbols}_symbols.json')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run SimpleTFT parallel training with CUDA streams")
    parser.add_argument('--num_symbols', type=int, default=4, help='Number of symbols to train')
    parser.add_argument('--multi_gpu', action='store_true', help='Use multiple GPUs if available')
    args = parser.parse_args()

    run_parallel_experiment(num_symbols=args.num_symbols, multi_gpu=args.multi_gpu)
